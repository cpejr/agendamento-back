...
module.exports = {
    ...
    async update(request, response) {
        try{
            ...
            const data = await Data.update({id},
                {
                    id_equipament,
                    temperature,
                    voltage,
            ...
            console.log(err);
            return response.status(500).json({ notification: "Internal server error while trying to update the data" });
        }
    },
    ...
    async delete(request, response) {
        try{
            ...
            return response.status(200).json({ notification: "Successfully deleted item"});
        } catch (err) {
            console.log(err);
            return response.status(500).json({ notification: "Internal server error while trying to delete the data" });
        }
    },
